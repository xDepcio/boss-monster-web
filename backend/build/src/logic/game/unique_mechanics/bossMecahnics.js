...
class GainOneGoldEveryTimeMonsterDungeonIsBuild extends BossMechanic {
    ...
    handleGameEvent(event) {
        if (!super.handleGameEvent(event))
            return;
...
class BoostEveryTrapDungeonFor1EnemiesCanPay1GoldToDeactivate extends BossMechanic {
    ...
    handleRankUp() {
        const cardAction = new CardAction("Zapłać 1 gold", [...this.bossCard.trackedGame.players].filter(player => player.id !== this.bossCard.owner.id), (playerThatUsed) => {
    ...
    disableForRound() {
        ...
        this.bossCard.trackedGame.addRoundModifier(new RoundModifer(() => {
