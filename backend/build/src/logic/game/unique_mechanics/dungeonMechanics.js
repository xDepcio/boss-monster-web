...
class EliminateHeroInDungeon extends DungeonMechanic {
    ...
    getHeroOnThisDungeon() {
        const hero = this.dungeonCard.owner.dungeonEntranceHeroes.find(hero => { var _a; return ((_a = hero.dungeonRoom) === null || _a === void 0 ? void 0 : _a.id) === this.dungeonCard.id; });
        return hero;
    }
}
...
class DrawSpellWhenPlayedSpell extends DungeonMechanic {
    constructor(dungeonCard, type, mechanicDescription) {
        super(dungeonCard, type, mechanicDescription);
...
class add1TreasureToAnyDungeonForThisRoundOnDestory extends DungeonMechanic {
    ...
    use() {
        if (!this.selectedTreasure) {
            ...
            this.dungeonCard.trackedGame.addRoundModifier(new RoundModifer(() => this.selectedPlayer.collectedTreasure[this.selectedTreasure] += 1, // TODO... add method on player to do this and this method should save new gameAction
...
class getOneDamageForEveryLuredHero extends DungeonMechanic {
    ...
    use() {
        this.dungeonCard.trackedGame.addRoundModifier(new RoundModifer(() => this.dungeonCard.damage += 1, () => this.dungeonCard.damage -= 1));
    }
    handleGameEvent(event) {
...
class Pay1GoldToDrawSpellWhenAnyDungeonDestroyed extends DungeonMechanic {
    ...
    requestPlayerSelect() {
        this.dungeonCard.owner.setRequestedSelection(new SelectionRequestOneFromGivenList(this.dungeonCard.owner, `Przeciwnik zniszczył komnatę, czy chcesz zapłacić 1 golda aby dobrać czar? (Zdolność karty '${this.dungeonCard.name}')`, ['tak', 'nie'], this));
    }
    receiveSelectionData(data) {
